...
public final class HeatingForgeVoxelRenderer {
    ...
    public static void render(
        ...
        renderModel(model, sprite, poseStack, bufferSource.getBuffer(RenderType.cutout()), baseLight, packedOverlay, baseColor(clampedHeat));
        if (clampedHeat > 0.05F) {
            poseStack.pushPose();
            float edgeScale = 1.03F + clampedHeat * 0.035F;
            poseStack.scale(edgeScale, edgeScale, edgeScale);
            renderModel(model, sprite, poseStack, bufferSource.getBuffer(RenderType.translucent()), LightTexture.FULL_BRIGHT, packedOverlay, edgeColor(clampedHeat));
            poseStack.popPose();
        }
    }

    private static void renderModel(HeatingForgeVoxelModel model, TextureAtlasSprite sprite, PoseStack poseStack, VertexConsumer consumer, int light, int overlay, int color) {
        for (HeatingForgeVoxelModel.Element element : model.elements()) {
            ...
            for (var entry : element.faces().entrySet()) {
                renderFace(entry.getKey(), entry.getValue(), sprite, poseStack, consumer, light, overlay, color, x1, y1, z1, x2, y2, z2);
            }
        }
    }
    ...
    private static void renderFace(
        ...
        float[] uv = face.uv();
        float u0 = sprite.getU(uv[0]);
        float v0 = sprite.getV(uv[1]);
        float u1 = sprite.getU(uv[2]);
        float v1 = sprite.getV(uv[3]);
